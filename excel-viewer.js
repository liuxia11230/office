...
class ExcelViewer {
    constructor(options) {
        ...
        this.sharedStrings = [];
        this.numFmts = {};
        this.currentMaxCol = 0;
        this.currentColWidths = {};
        this.currentMDW = 7;
    ...
    async loadFile(file) {
        try {
            ...
            await this.parseSharedStrings(zip);
            const workbook = await this.parseWorkbook(zip);
            
            console.log('解析完成:', {
                主题颜色: this.themeColors,
                字体数量: this.fonts.length,
                填充数量: this.fills.length,
                样式数量: this.cellXfs.length
            });
            
            this.processWorkbook(workbook, zip);
            
        } catch (err) {
            console.error('解析失败:', err);
            throw new Error('无法解析文件: ' + err.message);
        }
    }
    ...
    async parseTheme(zip) {
        this.themeColors = { ...this.DEFAULT_THEME };
        ...
        for (const [name, index] of Object.entries(colorMap)) {
            ...
            if (elem) {
                ...
                if (srgb) {
                    this.themeColors[index] = srgb.getAttribute('val');
                } else if (sys) {
                    this.themeColors[index] = sys.getAttribute('lastClr') || this.DEFAULT_THEME[index];
                }
            }
        }
        
        console.log('主题颜色:', this.themeColors);
    }
    ...
    async parseStyles(zip) {
        ...
        doc.querySelectorAll('numFmt').forEach(nf => {
            ...
            if (id && code) this.numFmts[id] = code;
        });
        ...
        doc.querySelectorAll('fonts > font').forEach(fontElem => {
            ...
            if (color) font.color = this.parseColor(color);
            ...
            ['left', 'right', 'top', 'bottom'].forEach(side => {
                ...
                if (sideElem) {
                    ...
                    if (style) {
                        ...
                        if (color) border[side].color = this.parseColor(color);
                    }
                }
            });
        ...
        doc.querySelectorAll('cellXfs > xf').forEach(xf => {
            ...
            this.cellXfs.push(style);
        });

        // 合并基础样式中的对齐（如果 cellXfs 中没有对齐信息）
        this.cellXfs.forEach(style => {
        ...
        console.log('cellXfs 数量:', this.cellXfs.length);
    }

    parseColor(colorElem) {
    ...
    async parseSharedStrings(zip) {
        ...
        doc.querySelectorAll('si').forEach(si => {
            this.sharedStrings.push(this.extractRichText(si));
        });
    }

    async parseWorkbook(zip) {
    ...
    parseHyperlinks(sheetDoc, relsDoc) {
        const map = new Map();
        ...
        sheetDoc.querySelectorAll('hyperlink').forEach(link => {
            ...
            if (!ref) return;
            this.expandRangeRefs(ref).forEach(cellRefStr => {
    ...
    expandRangeRefs(ref) {
        ...
        const s = this.cellRef(start);
        const e = this.cellRef(end);
        const refs = [];
        for (let r = s.r; r <= e.r; r++) {
            for (let c = s.c; c <= e.c; c++) {
                refs.push(`${this.colName(c)}${r}`);
            }
        }
        return refs;
    }
    ...
    buildHyperlinkHtml(text, target) {
        const str = String(text || '');
        const match = str.match(/https?:\/\/\S+/);
        if (!match) return this.escapeHtml(str);
        const url = match[0];
        const href = target || url;
        const before = str.slice(0, match.index);
        const after = str.slice(match.index + url.length);
        return `${this.escapeHtml(before)}<a class="cell-hyperlink" href="${this.escapeHtml(href)}" target="_blank" rel="noopener noreferrer">${this.escapeHtml(url)}</a>${this.escapeHtml(after)}`;
    }
    ...
    resolveColor(colorObj) {
        ...
        if (colorObj.theme !== undefined) {
            let baseColor = this.themeColors[colorObj.theme] || this.DEFAULT_THEME[colorObj.theme];
            if (baseColor) {
                let color = '#' + baseColor;
                if (colorObj.tint && colorObj.tint !== 0) {
                    color = this.applyTint(color, colorObj.tint);
                }
                return color;
            }
        }
    ...
    processWorkbook(workbook, zip) {
        ...
        this.setupTabs(workbook.sheets, zip);
        
        if (workbook.sheets.length > 0) {
            this.loadSheet(workbook.sheets[0], zip);
        }
    }

    setupTabs(sheets, zip) {
    ...
    async loadSheet(sheetInfo, zip) {
        ...
        if (!sheetFile) {
            this.tableWrapper.innerHTML = '<div style="padding:2rem;text-align:center;">无法加载工作表</div>';
            return;
        }
        ...
        this.renderSheet(doc, sheetRelsDoc);
    }
    ...
    renderSheet(doc, sheetRelsDoc) {
        ...
        // 解析冻结窗格信息
        const freezeInfo = this.parseFreezePanes(doc);
        ...
        // 默认字体（通常是 fonts[0]）
        if (this.fonts[0]) {
            table.style.fontFamily = this.getFontFamily(this.fonts[0].name, false);
            table.style.fontSize = (this.fonts[0].size || 11) + 'pt';
            table.style.fontWeight = this.getFontWeight(this.fonts[0].name, this.fonts[0].bold);
        }
        // 计算默认字体的最大数字宽度（MDW）
        this.currentMDW = this.measureMaxDigitWidth();

        // 超链接主题色
        const hlinkColor = this.resolveColor({ theme: 10 }) || '#0563c1';
        ...
        // 超链接映射（ref -> { target })
        const hyperlinkMap = this.parseHyperlinks(doc, sheetRelsDoc);
        const mergeSpanMap = new Map();
        doc.querySelectorAll('mergeCell').forEach(mc => {
            const ref = mc.getAttribute('ref');
            if (ref) {
                const [start, end] = ref.split(':');
                const s = this.cellRef(start), e = this.cellRef(end);
                merges.set(`${s.r},${s.c}`, {
                    rowspan: e.r - s.r + 1,
                    colspan: e.c - s.c + 1,
                    isMaster: true
        ...
        if (rows.length === 0) {
            this.tableWrapper.innerHTML = '<div style="padding:2rem;text-align:center;color:#666;">空工作表</div>';
            return;
        }
        ...
        rows.forEach(row => {
            row.querySelectorAll('c').forEach(c => {
                const ref = this.cellRef(c.getAttribute('r'));
                if (ref.c > maxCol) maxCol = ref.c;
            });
        });
        maxCol = Math.min(maxCol, 100);
        this.currentMaxCol = maxCol;
        this.currentColWidths = { ...colWidths };
        ...
        for (let c = 1; c <= maxCol; c++) {
            ...
            const w = this.calcColumnWidthPx(colWidths[c] || 8.43);
        ...
        rows.forEach(rowElem => {
            ...
            for (let c = 1; c <= maxCol; c++) {
                ...
                const cellStyle = (resolvedStyleIndex !== null && resolvedStyleIndex !== undefined)
                    ? this.cellXfs[resolvedStyleIndex]
                    : null;

                // 文本溢出（Excel 允许文本溢出到右侧连续空单元格）
                let overflowEndCol = null;
                if (cellElem) {
                    overflowEndCol = this.getOverflowEndCol(
                        cellElem,
                        resolvedStyleIndex,
                        rowNum,
                        c,
                ...
                if (cellElem) {
                    this.renderCell(td, cellElem, resolvedStyleIndex, rowNum, c, resolveStyleIndex, overflowEndCol, hyperlinkMap);
                } else if (resolvedStyleIndex !== null && !Number.isNaN(resolvedStyleIndex)) {
                    // 空单元格也要应用样式（尤其是边框）
                    this.renderCell(td, null, resolvedStyleIndex, rowNum, c, resolveStyleIndex, overflowEndCol, hyperlinkMap);
                }
                ...
                if (merge && merge.isMaster) {
                    this.applyMergedBorders(td, rowNum, c, merge, resolveStyleIndex);
                }
                ...
                // 更新冻结列偏移
                if (c <= freezeInfo.frozenCols) {
                    const colWidth = this.calcColumnWidthPx(colWidths[c] || 8.43);
        ...
        this.adjustAutoRowHeights(table, rowHeights, defaultRowHeight, freezeInfo);
        ...
        // 绑定悬停与选中效果
        this.bindHoverEffect(table);
        this.bindSelectionEffect(table);
        
        this.cellInfo.textContent = `${lastRow} 行 × ${maxCol} 列`;
    }

    renderCell(td, cellElem, styleIndex, rowNum, colNum, resolveStyleIndex, overflowEndCol, hyperlinkMap) {
        const resolvedStyleIndex = (styleIndex !== undefined && styleIndex !== null && !Number.isNaN(styleIndex))
            ...
            : (cellElem && cellElem.getAttribute('s') !== null)
                ...
                : 0;
        const cellStyle = this.cellXfs[resolvedStyleIndex];
        ...
        if (cellElem) {
            ...
            if (type === 's' && value) {
                ...
                if (isElem) {
                    value = this.extractRichText(isElem);
                }
            }
            ...
            if (!type || type === 'n') {
                ...
                if (!isNaN(numVal) && cellStyle) {
                    ...
                    value = this.formatNumber(numVal, fmtId);
                }
                td.classList.add('cell-number');
            }
            
            const ref = `${this.colName(colNum)}${rowNum}`;
            ...
            const verticalAlign = cellStyle && cellStyle.alignment && cellStyle.alignment.vertical;
            const needWrapper = isFixedHeight && (wrapText || (verticalAlign && verticalAlign !== 'bottom'));
            
            if (needWrapper) {
                // 固定高度 + (wrapText 或需要垂直对齐)：使用包装容器来限制高度并实现垂直对齐
                ...
                `;
                if (hyperlink) {
                    contentWrapper.innerHTML = this.buildHyperlinkHtml(value, hyperlink.target);
                } else {
                ...
                // 非固定高度或固定高度但垂直对齐为 bottom（默认）：正常渲染
                if (hyperlink) {
                    td.innerHTML = this.buildHyperlinkHtml(value, hyperlink.target);
                } else {
            ...
            if (overflowEndCol && overflowEndCol > colNum) {
                this.applyOverflowOverlay(td, value, colNum, overflowEndCol);
            }
        }
        
        // === 应用样式（无论是否有值） ===
        this.applyCellStyle(td, cellStyle, rowNum, colNum, resolveStyleIndex);
    }

    applyOverflowOverlay(td, text, startCol, endCol) {
        if (!text) return;
        ...
        td.textContent = '';
        td.appendChild(span);
        const width = this.calcOverflowWidthPx(startCol, endCol);
        span.style.width = width + 'px';
    }

    calcOverflowWidthPx(startCol, endCol) {
        let w = 0;
        for (let c = startCol; c <= endCol; c++) {
            const cw = this.calcColumnWidthPx(this.currentColWidths[c] || 8.43);
            w += cw;
        }
        return w;
    }

    measureTextWidth(text, cellStyle) {
        if (!text) return 0;
        if (!this._measureTextWidthCtx) {
            const canvas = document.createElement('canvas');
            this._measureTextWidthCtx = canvas.getContext('2d');
        }
        const ctx = this._measureTextWidthCtx;
        const font = this.getFontSpec(cellStyle);
        ctx.font = font;
        return ctx.measureText(String(text)).width;
    }

    measureMaxDigitWidth() {
        const ctx = this._measureTextWidthCtx || (this._measureTextWidthCtx = document.createElement('canvas').getContext('2d'));
        ctx.font = this.getFontSpec(null);
    ...
    getFontSizePx(cellStyle) {
        const fontId = cellStyle ? cellStyle.fontId : null;
        const font = (fontId !== null && fontId !== undefined) ? this.fonts[fontId] : null;
        const fontSizePt = (font && font.size) ? font.size : ((this.fonts[0] && this.fonts[0].size) || 11);
    ...
    getFontSpec(cellStyle) {
        const fontId = cellStyle ? cellStyle.fontId : null;
        const font = (fontId !== null && fontId !== undefined) ? this.fonts[fontId] : null;
        const fontName = (font && font.name) || (this.fonts[0] && this.fonts[0].name) || '等线';
        const fontWeight = this.getFontWeight(fontName, font && font.bold);
        const fontStyle = (font && font.italic) ? 'italic' : 'normal';
        const fontSizePt = (font && font.size) ? font.size : ((this.fonts[0] && this.fonts[0].size) || 11);
        const fontSizePx = (fontSizePt * 96) / 72;
        return `${fontStyle} ${fontWeight} ${fontSizePx}px ${this.getFontFamily(fontName, font && font.bold)}`;
    }

    getOverflowEndCol(cellElem, styleIndex, rowNum, colNum, cellMap, mergeSpanMap, resolveStyleIndex) {
        ...
        const cellStyle = (styleIndex !== null && styleIndex !== undefined) ? this.cellXfs[styleIndex] : null;
        const wrapText = cellStyle && cellStyle.alignment && cellStyle.alignment.wrapText;
        ...
        const text = this.getCellDisplayText(cellElem, cellStyle);
        if (!text) return null;

        // 如果当前单元格在合并区域内，禁用溢出
        if (mergeSpanMap && mergeSpanMap.has(`${rowNum},${colNum}`)) return null;

        const cellWidth = this.calcColumnWidthPx(this.currentColWidths[colNum] || 8.43);
        const textWidth = this.measureTextWidth(text, cellStyle);
        ...
        // 仅允许溢出到右侧连续空单元格（无值）
        let endCol = colNum;
        for (let c = colNum + 1; c <= this.currentMaxCol; c++) {
            const key = `${rowNum},${c}`;
            if (mergeSpanMap && mergeSpanMap.has(key)) break;
            const neighbor = cellMap[c];
            if (neighbor) {
                const neighborText = this.getCellDisplayText(neighbor, resolveStyleIndex ? resolveStyleIndex(rowNum, c) : null);
    ...
    getCellDisplayText(cellElem, styleIndex) {
        if (!cellElem) return '';
        const type = cellElem.getAttribute('t');
        const vElem = cellElem.querySelector('v');
        let value = vElem ? vElem.textContent : '';
    ...
    bindSelectionEffect(table) {
        ...
        table.addEventListener('click', (e) => {
            ...
            // 调试选中单元格的样式/溢出判断
            this.debugSelectedCell(table, row, col);
            
            // 更新顶部单元格信息栏
            this.updateCellInfoBar(table, row, col);
        });
    }

    updateCellInfoBar(table, row, col) {
        if (!row || !col || !this.cellPosition || !this.cellInfoContent) return;
        
        const colNum = parseInt(col);
        const rowNum = parseInt(row);
        const cellRef = `${this.colName(colNum)}${rowNum}`;
        this.cellPosition.textContent = cellRef;
        ...
        const cellStyle = (styleIndex !== null && styleIndex !== undefined) ? this.cellXfs[styleIndex] : null;
        const text = cellElem ? this.getCellDisplayText(cellElem, cellStyle) : '';
        
        this.cellInfoContent.textContent = text || '(空)';
    }

    bindCellInfoBarResize() {
        if (!this.cellInfoBar) return;
        const resizeHandle = this.cellInfoBar.querySelector('.cell-info-resize-handle');
        ...
        resizeHandle.addEventListener('mousedown', (e) => {
            isResizing = true;
            startY = e.clientY;
            startHeight = this.cellInfoBar.offsetHeight;
            this.cellInfoBar.style.transition = 'none';
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            this.cellInfoBar.style.height = `${newHeight}px`;
        });

        document.addEventListener('mouseup', () => {
            if (isResizing) {
                isResizing = false;
                this.cellInfoBar.style.transition = '';
    ...
    debugSelectedCell(table, row, col) {
        ...
        const cellStyle = (styleIndex !== null && styleIndex !== undefined) ? this.cellXfs[styleIndex] : null;
        const text = cellElem ? this.getCellDisplayText(cellElem, cellStyle) : '';
        const cellWidth = this.calcColumnWidthPx((table._colWidths && table._colWidths[col]) || 8.43);
        const textWidth = this.measureTextWidth(text, cellStyle);
        const overflowEnd = cellElem
            ? this.getOverflowEndCol(cellElem, styleIndex, parseInt(row), parseInt(col), {}, table._mergeSpanMap, resolveStyleIndex)
            : null;
    ...
    applyCellStyle(td, cellStyle, rowNum, colNum, resolveStyleIndex) {
        ...
        // 字体
        const font = this.fonts[cellStyle.fontId];
        if (font) {
            const fontName = font.name || (this.fonts[0] && this.fonts[0].name) || '等线';
            const fontFamily = this.getFontFamily(fontName, font.bold);
            const fontWeight = this.getFontWeight(fontName, font.bold);
        ...
        // 填充（背景色）
        const fill = this.fills[cellStyle.fillId];
        if (fill && fill.type === 'pattern') {
            if (fill.pattern === 'solid' && fill.fgColor) {
                const bgColor = this.resolveColor(fill.fgColor);
        ...
        // 边框
        const border = this.borders[cellStyle.borderId];
        const applyBorderSide = (side, bs) => {
            if (!bs || !bs.style) return;
            // 忽略外侧列边框（第1列左边、最后一列右边）
            if (side === 'left' && colNum === 1) return;
            if (side === 'right' && this.currentMaxCol && colNum === this.currentMaxCol) return;
            let w = '1px', s = 'solid';
            switch (bs.style) {
            ...
            const c = bs.color ? this.resolveColor(bs.color) : '#000';
        ...
        if (typeof resolveStyleIndex === 'function') {
            ...
            if (!td.style.borderLeft && colNum > 1) {
                ...
                if (leftStyleIdx !== null && leftStyleIdx !== undefined) {
                    const leftStyle = this.cellXfs[leftStyleIdx];
                    const leftBorder = leftStyle ? this.borders[leftStyle.borderId] : null;
            ...
            if (!td.style.borderRight) {
                ...
                if (rightStyleIdx !== null && rightStyleIdx !== undefined) {
                    const rightStyle = this.cellXfs[rightStyleIdx];
                    const rightBorder = rightStyle ? this.borders[rightStyle.borderId] : null;
            ...
            if (!td.style.borderTop && rowNum > 1) {
                ...
                if (topStyleIdx !== null && topStyleIdx !== undefined) {
                    const topStyle = this.cellXfs[topStyleIdx];
                    const topBorder = topStyle ? this.borders[topStyle.borderId] : null;
            ...
            if (!td.style.borderBottom) {
                ...
                if (bottomStyleIdx !== null && bottomStyleIdx !== undefined) {
                    const bottomStyle = this.cellXfs[bottomStyleIdx];
                    const bottomBorder = bottomStyle ? this.borders[bottomStyle.borderId] : null;
    ...
    applyMergedBorders(td, rowNum, colNum, merge, resolveStyleIndex) {
        ...
        const applyBorderSide = (side, bs) => {
            ...
            if (side === 'right' && this.currentMaxCol && endCol === this.currentMaxCol) return;
            let w = '1px', s = 'solid';
            switch (bs.style) {
                case 'thin': w = '1px'; break;
                case 'medium': w = '2px'; break;
        ...
        const getBorderForCell = (r, c) => {
            ...
            const style = this.cellXfs[styleIdx];
            if (!style) return null;
            return this.borders[style.borderId] || null;
        };
    ...
    formatNumber(value, fmtId) {
        ...
        let fmt = this.numFmts[fmtId] || builtIn[fmtId] || 'General';
